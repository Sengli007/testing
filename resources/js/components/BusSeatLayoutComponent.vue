    ...
    <div class="container">
        <div class="row">
            <div class="card bg-primary text-white col-lg-12 mb-5">
                <div class="card-body">
                    <h4> Bus Info and Bus Seat Entry Form</h4>
                </div>
            </div>
        </div>
        <div class="row">
...
<script>
export default {
   props: ['bus_seat_entry','bus_layout'],
   data(){
       return {
          seat_ary:[],
           seat_rows:'',
   ...
   methods:{
        ...
        console.log(this.tables);
           ...
           var data= {
                bus_seat_ary: this.tables,
                busId: this.bus_seat_entry.id
           }

           axios.post('/busSeatEntry', data ).then (resp => {
               if(resp.data != '') {
                    this.bus_id= resp.data;
                    window.location.href = '/busSeat/'+ this.bus_id +'/Show';
               }
           })
